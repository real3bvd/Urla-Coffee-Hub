...
router.get("/menu", async (req: Request, res: Response) => {
  try {
    ...
    const [{ db }, { menuCategoriesTable, menuItemsTable }, { asc }] =
      await Promise.all([
        import("@workspace/db"),
    ...
    const categories = await db
      ...
      .orderBy(
        asc(menuCategoriesTable.sortOrder),
        asc(menuCategoriesTable.createdAt),
      );
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      if (!useDatabase) {
        res.json(
          await addLocalCategory({ nameTr, nameEn, sortOrder: sortOrder ?? 0 }),
        );
        return;
      }
...
router.delete(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const [{ db }, { menuCategoriesTable, menuItemsTable }, { eq }] =
        await Promise.all([
          import("@workspace/db"),
          import("@workspace/db/schema"),
          import("drizzle-orm"),
...
router.post(
  ...
  requireAdmin,
  async (req: Request, res: Response) => {
    const { categoryId, nameTr, nameEn, descTr, descEn, photoUrl, sortOrder } =
      req.body as {
        categoryId?: number;
        nameTr?: string;
        nameEn?: string;
    ...
    if (!categoryId || !nameTr || !nameEn) {
      res
        .status(400)
        .json({ error: "categoryId, nameTr, nameEn are required" });
      return;
    }
    try {
      if (!useDatabase) {
...
router.put(
  ...
  async (req: Request, res: Response) => {
    const id = Number(req.params.id);
    const { nameTr, nameEn, descTr, descEn, photoUrl, sortOrder, categoryId } =
      req.body as {
        nameTr?: string;
        nameEn?: string;
        descTr?: string;
