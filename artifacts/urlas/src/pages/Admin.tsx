...
function useAdminSession() {
  ...
  async function login(pw: string) {
    ...
    try {
      ...
      if (r.status === 401) {
        setError("Hatalı şifre / Wrong password");
        return;
      }
      if (!r.ok) {
        setError("Giris yapilamadi / Login failed");
        return;
      }
...
async function uploadImage(file: File, token: string): Promise<string> {
  ...
  const put = await fetch(apiUrl(uploadURL), {
    method: "PUT",
    body: file,
...
function Field({
  ...
  const cls =
    "w-full border border-border rounded px-3 py-2 text-sm bg-background focus:outline-none focus:ring-1 focus:ring-olive font-sans";
  return (
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          <label className="text-[10px] text-olive font-sans uppercase tracking-widest mb-1 block">
            TR
          </label>
          {multiline ? (
            <textarea
...
function Toast({ msg, ok }: { msg: string; ok: boolean }) {
  return (
    <div
      className={`fixed bottom-6 right-6 z-50 px-5 py-3 rounded text-sm font-sans shadow-lg ${ok ? "bg-olive text-white" : "bg-red-600 text-white"}`}
    >
      {msg}
    </div>
  );
...
function GeneralTab({ token }: { token: string }) {
  const keys: Array<{ key: string; label: string; multiline?: boolean }> = [
    ...
    {
      key: "about.p1",
      label: "Hakkımızda Paragraf 1 / About P1",
      multiline: true,
    },
    {
      key: "about.p2",
      label: "Hakkımızda Paragraf 2 / About P2",
      multiline: true,
    },
  ...
  const [content, setContent] = useState<ContentRecord>(() =>
    contentDefaultsFor(contentKeys),
  );
  ...
  const fieldDefs: Array<{ key: string; label: string; multiline?: boolean }> =
    [
      ...
      { key: "visit.phone", label: "Telefon / Phone" },
      {
        ...
        label: "Instagram Kullanıcı Adı / Instagram Handle",
      },
    ];
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="text-xs text-muted-foreground font-sans p-3 bg-muted/40 rounded">
        Instagram için sadece kullanıcı adı yazın (örn: urlascoffee) — @ işareti
        olmadan.
        <br />
        ...
        {images.map((img) => (
          ...
          >
            ...
            <div className="p-3 space-y-2">
              <div>
                <label className="text-[10px] text-olive font-sans uppercase tracking-widest">
                  Alt TR
                </label>
                <input
...
function MenuTab({ token }: { token: string }) {
  ...
  function load() {
    fetch(apiUrl(`${API}/menu`))
      .then((r) => r.json())
      .then((d: { categories: ApiCategory[]; items: ApiItem[] }) => {
        const cats = d.categories?.length
          ? d.categories
          : DEFAULT_MENU.categories;
        const itms = d.items?.length
          ? d.items.map((item) => ({
              ...item,
      ...
      .catch(() => {
        ...
        setCatEdits(
          Object.fromEntries(
            DEFAULT_MENU.categories.map((c) => [
              c.id,
              { nameTr: c.nameTr, nameEn: c.nameEn },
            ]),
          ),
        );
        setItemEdits(
          Object.fromEntries(
            DEFAULT_MENU.items.map((i) => [
              i.id,
              {
                nameTr: i.nameTr,
                nameEn: i.nameEn,
  ...
  async function addCategory() {
    ...
    try {
      await apiPost(
        "/menu/categories",
        { ...newCat, sortOrder: categories.length },
        token,
      );
  ...
  async function deleteCategory(id: number) {
    if (
      !confirm(
        "Kategori ve tüm ürünlerini sil? / Delete category and all its items?",
      )
    )
      return;
    try {
    ...
    try {
      await apiPost(
        "/menu/items",
        {
          ...newItem,
          categoryId,
          sortOrder: items.filter((i) => i.categoryId === categoryId).length,
        },
        token,
      );
  ...
  async function uploadItemPhoto(item: ApiItem, file: File) {
    try {
      ...
      setItems((prev) =>
        prev.map((i) => (i.id === item.id ? { ...i, photoUrl: imgUrl } : i)),
      );
      showToast("Fotoğraf yüklendi!");
    } catch {
      showToast("Yükleme hatası", false);
    }
  }

  const inputCls =
    "w-full border border-border rounded px-2 py-1.5 text-sm bg-background focus:outline-none focus:ring-1 focus:ring-olive font-sans";

  return (
    <div className="space-y-6">
      <div className="border border-border rounded p-4 space-y-3">
        <p className="text-xs font-sans uppercase tracking-widest text-olive">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input
              ...
              value={newCat.nameTr}
              onChange={(e) =>
                setNewCat((p) => ({ ...p, nameTr: e.target.value }))
              }
            />
          </div>
          <div>
            ...
            <input
              ...
              value={newCat.nameEn}
              onChange={(e) =>
                setNewCat((p) => ({ ...p, nameEn: e.target.value }))
              }
            />
          </div>
        </div>
        ...
        return (
          ...
          >
            <div className="flex items-center gap-3 px-4 py-3 bg-muted/30">
              ...
              >
                {isExpanded ? (
                  <ChevronUp className="w-4 h-4 text-olive shrink-0" />
                ) : (
                  <ChevronDown className="w-4 h-4 text-olive shrink-0" />
                )}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setCatEdits((p) => ({
                          ...p,
                          [cat.id]: { ...p[cat.id], nameTr: e.target.value },
                        }))
                      }
                      onBlur={(e) =>
                        updateCategory(
                          cat.id,
                          e.target.value,
                          catEdits[cat.id]?.nameEn ?? cat.nameEn,
                        )
                      }
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setCatEdits((p) => ({
                          ...p,
                          [cat.id]: { ...p[cat.id], nameEn: e.target.value },
                        }))
                      }
                      onBlur={(e) =>
                        updateCategory(
                          cat.id,
                          catEdits[cat.id]?.nameTr ?? cat.nameTr,
                          e.target.value,
                        )
                      }
                    />
                    ...
                    >
                      <div className="flex items-start gap-3">
                        <div className="w-16 h-16 shrink-0 bg-muted rounded overflow-hidden relative group">
                          {item.photoUrl ? (
                            <img
                              src={mediaUrl(item.photoUrl) ?? item.photoUrl}
                              alt={item.nameTr}
                              className="w-full h-full object-cover"
                            />
                          ) : (
                          ...
                          <input
                            ...
                            className="hidden"
                            ref={(el) => {
                              uploadRefs.current[item.id] = el;
                            }}
                            onChange={(e) => {
                              const f = e.target.files?.[0];
                              if (f) uploadItemPhoto(item, f);
                              e.target.value = "";
                            }}
                          />
                        </div>
                        <div className="flex-1 grid grid-cols-2 gap-2">
                          <div>
                            <label className="text-[10px] text-olive font-sans uppercase tracking-widest block mb-1">
                              TR Ad
                            </label>
                            <input
                              className={inputCls}
                              ...
                              onBlur={(e) =>
                                updateItem(item, "nameTr", e.target.value)
                              }
                            />
                          </div>
                          <div>
                            ...
                            <input
                              ...
                              onBlur={(e) =>
                                updateItem(item, "nameEn", e.target.value)
                              }
                            />
                          </div>
                          <div>
                            <label className="text-[10px] text-muted-foreground font-sans uppercase tracking-widest block mb-1">
                              Açıklama TR
                            </label>
                            <input
                              className={inputCls}
                              ...
                              onBlur={(e) =>
                                updateItem(item, "descTr", e.target.value)
                              }
                            />
                          </div>
                          <div>
                            ...
                            <input
                              ...
                              onBlur={(e) =>
                                updateItem(item, "descEn", e.target.value)
                              }
                            />
                          </div>
                        </div>
                  ...
                  <div className="border border-olive/40 rounded p-3 space-y-3 bg-olive/5">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <input
                        className={inputCls}
                        placeholder="Ad TR"
                        value={newItem.nameTr}
                        onChange={(e) =>
                          setNewItem((p) => ({ ...p, nameTr: e.target.value }))
                        }
                      />
                      <input
                        className={inputCls}
                      ...
                      <input
                        ...
                        value={newItem.descTr}
                        onChange={(e) =>
                          setNewItem((p) => ({ ...p, descTr: e.target.value }))
                        }
                      />
                      <input
                        className={inputCls}
                        ...
                        onChange={(e) =>
                          setNewItem((p) => ({ ...p, descEn: e.target.value }))
                        }
                      />
                    </div>
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex items-center gap-2 px-3 py-2 bg-olive text-white text-xs uppercase tracking-widest font-sans rounded hover:bg-olive/80 transition-colors"
                      >
                      ...
                      <button
                        ...
                        className="px-3 py-2 border border-border text-xs uppercase tracking-widest font-sans rounded hover:border-olive transition-colors"
                      >
                        <X className="w-3 h-3" />
                      </button>
                    </div>
      ...
      {categories.length === 0 && (
        <div className="text-center py-12 text-muted-foreground text-sm font-sans">
          Henüz kategori yok. Yukarıdan ekleyin. / No categories yet. Add one
          above.
        </div>
      )}
      {toast && <Toast msg={toast.msg} ok={toast.ok} />}
    ...
    <div className="min-h-screen bg-background">
      <nav className="sticky top-0 z-30 bg-background border-b border-border/60 px-4 sm:px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link
            href="/"
            className="flex items-center gap-2 text-xs text-muted-foreground hover:text-foreground font-sans uppercase tracking-widest transition-colors"
          >
